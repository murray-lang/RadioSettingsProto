...
namespace Config::DigitalInputs
{
  ...
  ResultCode fromJson(const TypedJson& json, Config::DigitalInputs::Fields& fields)
  {
    fields.type = type;
    fields.inputs.clear();
    ...
    if (json.config["inputs"]) {
      for (JsonVariantConst inputJson : json.config["inputs"].as<JsonArrayConst>()) {
        ...
        // DigitalInput::Fields diFields{};
        // result = DigitalInput::fromJson(typedJson.config, diFields);

        DigitalInput::Fields diFields{};
        result = DigitalInput::fromJson(inputJson, diFields);

        if (result != ResultCode::OK) return result;

        fields.inputs.emplace_back(diFields);
      }
    }
